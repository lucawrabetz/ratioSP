...
DENSE_FILE_NAMES = [str(i+4) + DENSE_BASE_FILENAME for i in range(FILE_NUM)]
SPARSE_FILE_NAMES = [str(i+4) + SPARSE_BASE_FILENAME for i in range(FILE_NUM)]
    ...
    while connected==False:
        ...
        for u in range(N):
            for v in range(u+1, N):
                ...
                if exists:
                    G.add_edge(u, v, cost=int(cost_list[counter]), tau=int(tau_list[counter]))
                    m += 1
                counter += 1
        ...
        with open(filename, 'w') as new: new.write(str(N) + " " + str(m) + "\n" + data)

    return

for i in range(FILE_NUM):
    write_graph(DENSE_FILE_NAMES[i], i+4)
    write_graph(SPARSE_FILE_NAMES[i], i+4, False)
